...
def gerar_grafico(conexao,
    ...
    nome_arquivo_saida = pasta_graficos / f"barra_tipo_horas_absenteismo_{mes_ano_filtro}.png"

    #data_inicio_sql = '2026-06-01'
    #data_fim_sql = '2026-06-30'

    # Ler a consulta SQL dentro do dicionário SCRIPTS_SQL do arquivo queries_absenteismo_geral.py diretamente para um DataFrame do Pandas
    ...
    if df.empty:
        ...
        #df_mes_desejado = df[df['MES_ANO'] == mes_ano_filtro]

        # Agrupando e sumarizando as colunas, pois estão separadas por regional ou centro de custo
        # para obter o valor total mensal
        #dados_mensal = df_mes_desejado.groupby('MES_ANO')[colunas_desejadas].sum().reset_index()

        # Ordena do maior absenteísmo para o menor
        # Se quiser limitar nos 20 ou 10 maiores, use o '.head(20 ou 10)'
        #media_mensal_simples = media_mensal_simples.sort_values(by='TAXA_ABSENTEISMO_PERC', ascending=False).head(20)
        #media_mensal_simples_centro_custo['NOME_CENTRO_CUSTO'] = media_mensal_simples_centro_custo['NOME_CENTRO_CUSTO'].astype(str)

        # 3. 'Derreter' o DataFrame (Wide -> Long)
        df_longo = pd.melt(
            df,
            id_vars=['MES_ANO'],
        ...
        # HUE: Define qual coluna diferencia as cores
        # Pallete: Passa o dicionário { 'Categoria': '#HEX' }
        # Legend=False: Oculta a legenda já que o eixo X é autoexplicativo
        ax = sns.barplot(data=df_longo, x='TIPO_HORAS', y='HORAS', palette=paleta_cores, hue='TIPO_HORAS',  legend=False)
        ...
        total_horas = df_longo['HORAS'].sum()
        ...
        for container in ax.containers:
            ...
            for v in container:
                ...
                texto = f"{altura_formatado} {percentual_formatado}"
                labels.append(texto)
            ax.bar_label(container, labels=labels, padding=4, fontsize=10, weight='bold')


        plt.title(f'Divisão das Horas | Mês: {mes_ano_extenso}', fontsize=14, pad=15, weight='bold')
        plt.xlabel("Tipo de Horas", fontsize=12, labelpad=10)
        plt.ylabel('Total de Horas', fontsize=12, labelpad=10)

        # Rotacionando os nomes dos centros de custo em 90 graus para não se cruzarem
        #plt.xticks(rotation=45, ha='right', fontsize=10)
        #plt.legend(title='Motivo', loc='upper right')
