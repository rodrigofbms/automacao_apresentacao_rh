...
paleta_cores = {'Faltas': "#D8002F", 'Atestados': '#F28E2B', 'Atrasos': "#DA9500", 'BancoHoras' :'#0EA5E9'}
...
def gerar_grafico(conexao,
    ...
    df = pd.read_sql(SCRIPTS_SQL["atrasos_por_mes"], conexao, params=[data_inicio_sql, data_fim_sql])
    ...
    if df.empty:
        ...
        total_horas_ponto = row.get("TOTAL_HORAS_ATRASO_PONTO", 0)
        custo_estimado_ponto = row.get("TOTAL_CUSTO_ESTIMADO_ATRASOS_PONTO_R$", 0)
        custo_estimado_banco = row.get("TOTAL_CUSTO_ESTIMADO_ATRASOS_BANCO_R$", 0)
        valor_descontado_folha = row.get("VALOR_DESCONTADO_FOLHA_R$", 0)
        ...
        cards = [
            # --- Linha Superior (2 Cards em destaque) ---
            {
                ...
                'valor': formatar_numero(valor= total_horas_ponto, prefixo= "",sufixo= "h"),
                'cor': paleta_cores['Atrasos']
            },
            {
                'rect': (0.52, 0.52, 0.38, 0.40),
                'titulo': 'Custo Estimado dos Atrasos\n do Ponto \n (Valor da Hora x Atrasos)',
                'valor': formatar_numero(valor= custo_estimado_ponto,prefixo= "R$ ",sufixo= ""),
                'cor': paleta_cores['Atrasos']
            },
            # --- Linha Inferior (2 Cards complementares) ---
            {
                ...
                'titulo': 'Custo Estimado dos Atrasos\n do Banco de Horas \n (Valor da Hora x Atrasos)',
                'valor': formatar_numero(valor= custo_estimado_banco,prefixo= "R$ ",sufixo= ""),
                'cor': paleta_cores['Atrasos']
            },
            {
                'rect': (0.52, 0.08, 0.38, 0.38),
                'titulo': 'Valor Descontado em Folha',
                'valor': formatar_numero(valor= valor_descontado_folha,prefixo= "R$ ",sufixo= ""),
                'cor': paleta_cores['Atrasos']
            },
        ]
        ...
        plt.tight_layout()
        assinar_grafico(plt.gcf(), cor_letra="gray", fontsize=12)
