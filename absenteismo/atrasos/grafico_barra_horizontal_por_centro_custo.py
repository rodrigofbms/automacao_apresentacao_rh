...
from absenteismo.atrasos.queries_absenteismo_atrasos import SCRIPTS_SQL 
...
def gerar_grafico(conexao,
    ...
    # Ler a consulta SQL dentro do dicionário SCRIPTS do arquivo queries_absenteismo_atrasos.py diretamente para um DataFrame do Pandas
    df = pd.read_sql(SCRIPTS_SQL["top_5_atrasos_por_centro_custo"], conexao, params=[data_inicio_sql, data_fim_sql])
    ...
    if df.empty:
        ...
        pasta_graficos = DIRETORIO_RAIZ / "absenteismo" / "atrasos" / "graficos"
        pasta_graficos.mkdir(parents=True, exist_ok=True)

        # Nome do arquivo de saída
        nome_arquivo_saida = pasta_graficos / f"top_5_atrasos_absenteismo_por_centro_custo_{mes_ano_filtro}.png"
        ...
        colunas_desejadas = ['MES_ANO', 'CENTRO_CUSTO', 'TOTAL_HORAS_ATRASO_PONTO']
        ...
        df_colunas_desejadas['TOTAL_HORAS_ATRASO_PONTO'] = pd.to_numeric(df_colunas_desejadas['TOTAL_HORAS_ATRASO_PONTO'], errors='coerce')
        ...
        df_ranking = df_colunas_desejadas.sort_values(by='TOTAL_HORAS_ATRASO_PONTO', ascending=False)
        ...
        total_horas = df_colunas_desejadas['TOTAL_HORAS_ATRASO_PONTO'].sum()
        ...
        plt.title(f'Top 5 Atrasos por Centro de Custo | Mês: {mes_ano_extenso}',
        ...
        min_val = df_ranking['TOTAL_HORAS_ATRASO_PONTO'].min()
        max_val = df_ranking['TOTAL_HORAS_ATRASO_PONTO'].max()
