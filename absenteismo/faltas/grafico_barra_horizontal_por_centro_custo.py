...
def gerar_grafico(conexao,
    ...
    nome_arquivo_saida = pasta_graficos / f"top_5_faltas_absenteismo_por_centro_custo_{mes_ano_filtro}.png"
    ...
    df = pd.read_sql(SCRIPTS_SQL["top_5_faltas_por_centro_custo"], conexao, params=[data_inicio_sql, data_fim_sql])
    ...
    if df.empty:
        ...
        colunas_desejadas = ['MES_ANO', 'CENTRO_CUSTO', 'TOTAL_HORAS_FALTA_PONTO']
        ...
        # Conversão das colunas para o tipo de cada uma
        ...
        df_colunas_desejadas['CENTRO_CUSTO'] = df_colunas_desejadas['CENTRO_CUSTO'].astype(str)
        ...
        # 2. Filtrar e Ordenar
        # -----------------------------------------------------------------------------
        #df_filtrado = df_colunas_desejadas[(df_colunas_desejadas['MES_ANO'] == mes_desejado)].copy()
        ...
        for container in ax.containers:
            labels = [
                formatar_numero(valor = val, sufixo = "h")
        ...
        plt.title(f'Top 5 Faltas por Centro de Custo | Mês: {mes_ano_extenso}',
