...
paleta_cores = {'Faltas': "#D8002F", 'Atestados': '#F28E2B', 'Atrasos': "#EDB948", 'BancoHoras' :'#0EA5E9'}
...
def gerar_grafico(conexao,
    ...
    if df.empty:
        ...
        total_horas_atestado = row.get("TOTAL_HORAS_ATESTADO", 0)
        custo_estimado_atestado = row.get("TOTAL_CUSTO_ESTIMADO_ATESTADOS_R$", 0)
        total_horas_folha = row.get("TOTAL_HORAS_FOLHA", 0)
        valor_descontado_folha = row.get("TOTAL_VALOR_DESCONTADO_FOLHA_R$", 0)
        ...
        cards = [
            # --- Linha Superior (2 Cards em destaque) ---
            {
                ...
                'valor': formatar_numero(valor= total_horas_atestado,prefixo= "", sufixo= "h"),
                'cor': paleta_cores['Atestados']
            },
            {
                'rect': (0.52, 0.52, 0.38, 0.40),
                'titulo': 'Custo Estimado\n dos Atestados\n (Valor da Hora x Atestados)',
                'valor': formatar_numero(valor= custo_estimado_atestado,prefixo= "R$ ", sufixo= ""),
                'cor': paleta_cores['Atestados']
            },
            # --- Linha Inferior (2 Cards complementares) ---
            {
                'rect': (0.10, 0.08, 0.38, 0.38),
                'titulo': 'Total de Horas na Folha',
                'valor': formatar_numero(valor= total_horas_folha, prefixo= "", sufixo= "h"),
                'cor': paleta_cores['Atestados']
            },
            {
