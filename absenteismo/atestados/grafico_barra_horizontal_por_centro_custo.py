...
from absenteismo.atestados.queries_absenteismo_atestados import SCRIPTS_SQL 
...
def gerar_grafico(conexao,
    ...
    pasta_graficos = DIRETORIO_RAIZ / "absenteismo" / "atestados" / "graficos"
    pasta_graficos.mkdir(parents=True, exist_ok=True)

    # Nome do arquivo de saída
    nome_arquivo_saida = pasta_graficos / f"top_5_atestados_absenteismo_por_centro_custo_{mes_ano_filtro}.png"

    # Ler a consulta SQL dentro do dicionário SCRIPTS do arquivo queries_absenteismo_atestados.py diretamente para um DataFrame do Pandas
    df = pd.read_sql(SCRIPTS_SQL["top_5_atestados_por_centro_custo"], conexao, params=[data_inicio_sql, data_fim_sql])
    ...
    if df.empty:
        ...
        colunas_desejadas = ['MES_ANO', 'CENTRO_CUSTO', 'TOTAL_HORAS_ATESTADO']
        ...
        # Converte para timestamp em pandas e depois com o formato timestamp usa o '.dt.strftime("%Y-%m")' para informar ao matplotlib
        # que essa coluna possui esse formato 'YYYY-mm' em datetime
        df_colunas_desejadas['MES_ANO'] = pd.to_datetime(df_colunas_desejadas['MES_ANO'], format="%Y-%m")

        # Converte para string
        df_colunas_desejadas['CENTRO_CUSTO'] = df_colunas_desejadas['CENTRO_CUSTO'].astype(str).str.replace('"', '').str.strip()

        # Converte para o tipo numerico
        df_colunas_desejadas['TOTAL_HORAS_ATESTADO'] = pd.to_numeric(df_colunas_desejadas['TOTAL_HORAS_ATESTADO'], errors='coerce')
        ...
        df_ranking = df_colunas_desejadas.sort_values(by='TOTAL_HORAS_ATESTADO', ascending=False)
        ...
        total_horas = df_colunas_desejadas['TOTAL_HORAS_ATESTADO'].sum()
        ...
        plt.title(f'Top 5 Atestados por Centro de Custo | Mês: {mes_ano_extenso}',
                fontsize=12, pad=15, weight='bold')
        plt.xlabel('Total de Horas', fontsize=10, labelpad=10)
        plt.ylabel('Centro de Custo', fontsize=10, labelpad=10)

        # Ajuste dos limites do Eixo X com base nos valores máximo e mínimo (margem visual para ambos os lados)
        min_val = df_ranking['TOTAL_HORAS_ATESTADO'].min()
        max_val = df_ranking['TOTAL_HORAS_ATESTADO'].max()
